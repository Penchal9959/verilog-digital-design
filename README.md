...
All three designs simulate cleanly under **Icarus Verilog 12.0** and pass their checks:
...
Each design ships a `run.do` script:
...
## Repairs made to the original code

These designs were written in 2021 and published without ever being simulated end to end. The
issues below were found and fixed while consolidating the repositories — they are listed openly
because the debugging is a more useful record than the code itself.
...
log in the old README depends on them — see [the multiplier README](multiplier-datapath-controller/README.md):
